		...
		for OUTPUT_PRODUCT in ${OUTPUT_PRODUCTS[@]}; do
			...
			for OUTPUT_STATISTIC in ${OUTPUT_STATISTICS[@]}; do
				...
				for OUTPUT_FILE_TYPE in ${OUTPUT_FILE_TYPES[@]}; do
					...
					echo '-' >> $LOAD_OUTPUT_PARAMETER_FILE #NODC_PFV5_QUAL_FILE
					
					run_seadas_batch_file ${SEABATCH_BIN_DIRECTORY}'/process/basename.sbf'
					...
					run_seadas_batch_file ${SEABATCH_BIN_DIRECTORY}'/process/load_dimensions.sbf'

					echo '-' >> $LOAD_OUTPUT_PARAMETER_FILE #AVHRR_SST_QUALITY
					...
					run_seadas_batch_file ${SEABATCH_BIN_DIRECTORY}'/process/load_output.sbf'
...
if [ $LOADED_FILE_TYPE = 'NODC_PFV5_SST' ]; then
	...
	while read LOADED_FILE; do
		...
		for OUTPUT_FILE_TYPE in ${OUTPUT_FILE_TYPES[@]}; do
			...
			run_seadas_batch_file ${SEABATCH_BIN_DIRECTORY}'/process/nodc_pfv5_qual_file_id.sbf'
